...
https://discuss.leetcode.com/topic/64462/c-python-0ms-o-log-n-2-time-o-1-space-super-easy-solution-with-detailed-explanations/2
Time Complexity: O( (logn) ^ 2 )
Here, logn is the number of digits in n, and it is also the number of replication appending zero to search detailedly.
Each such appending may introduce the increasement of prefix at most 10 times, and each increasement may require an O(logn)
to calculate the number of numbers that prefixed by the result.
...
public class KSmallestLexicographicalOrder {
    ...
    public static int findKthNumber(int n, int k) {
        ...
        k--;
        while (k > 0) {
            int steps = calculateStepsCount(n, curr, curr + 1);
            if (steps < k) {
                curr += 1;
                k -= steps;
            } else {
...
organize this flow to "steps += Math.min(n + 1, n2) - n1; n1 *= 10; n2 *= 10;"
     ...
     */
    private static int calculateStepsCount(int n, long n1, long n2) {
        int steps = 0;
        while (n1 < n) {
